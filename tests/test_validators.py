...
def test_validator_returns_section_word_counts():
    ...
    assert by_canon["abstract"]["word_count"] < 400  # fixture is short


def test_validator_flags_missing_required_section():
    """JPD requires a Clinical Implications section. The fixture has none,
    so it must be flagged as missing."""
    cfg = load_journal("jpd", config_dir=CFG_DIR)
    report = validate_manuscript(FIXTURE, cfg)
    missing = [m["canonical"] for m in report["missing_required"]]
    assert "clinical_implications" in missing


def test_validator_flags_overlong_abstract(tmp_path):
    """Synthesize a manuscript whose abstract exceeds 400 words; expect
    over_limit=True with limit=400."""
    in_path = tmp_path / "long_abstract.docx"
    doc = Document()
    doc.add_heading("Abstract", level=1)
    doc.add_paragraph("word " * 450)
    doc.save(in_path)
    cfg = load_journal("jpd", config_dir=CFG_DIR)
...
def test_validator_returns_total_word_count():
    ...
    assert report["total_word_limit"] == 3000
...
def test_validator_includes_original_heading_for_audit():
    ...
    so the audit trail in Task 14 can show 'Background renamed -> Introduction'."""
    cfg = load_journal("jpd", config_dir=CFG_DIR)
    report = validate_manuscript(FIXTURE, cfg)
